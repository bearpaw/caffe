...
namespace caffe {
/**
 * Image data layer for data with multiple layers
 * Usage
 * Prepare the 'source' file in following format
 * 		filename label1 label2 ... labeln
 * Define following param in `image_data_params`
 * 		label_num: n
 */
...
void ImageDataMultiLabelLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  while (infile >> filename) {
	...
	for (int l = 0; l < label_num_; ++l) {
		infile >> label;
		curlabels.push_back(label);
	}
    lines_.push_back(std::make_pair(filename, curlabels));
  }
  ...
  label_shape.push_back(label_num_);
...
template <typename Dtype>
void ImageDataMultiLabelLayer<Dtype>::InternalThreadEntry() {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    // Prefetch multi-label (YANG Wei)
    vector<Dtype> curlabels = lines_[lines_id_].second;
    offset = this->prefetch_label_.offset(item_id);
    for (int l = 0; l < label_num_; ++l) {
	  prefetch_label[offset + l] = curlabels[l];
	}
