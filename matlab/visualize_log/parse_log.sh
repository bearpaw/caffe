...
grep 'Testing net ' $1 | sed  's/.*Iteration \([[:digit:]]*\).*/\1/g' > aux0.txt
grep 'Test net output #0' aux.txt | awk '{print $11}' > acc.txt
grep 'Test net output #1' aux.txt | awk '{print $11}' > loss.txt
...
paste aux0.txt aux4.txt acc.txt loss.txt | column -t >> $LOG.test
rm aux.txt aux0.txt acc.txt loss.txt aux3.txt aux4.txt
